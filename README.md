...
The `Earthling.switchToTab` handler also emits `Target.detachedFromTarget` to the *current* client when its switch-source tab was a daemon-auto-opened blank that's now being closed — same rationale as the preemption path. Without this, Playwright's autoBlank `CRPage` survives the switch and any subsequent `Page.navigate` carries the autoBlank's cached `mainFrame.id` to the destination tab's chrome target, which answers `-32000 No frame with given id found`. The MCP backend's `Context._ensureInitialPagePooled` complements this by pooling the auto-attached page under the daemon-reported `Earthling.whoAmI.attachedTabId` (set ONCE at auto-attach time, cleared when that tab is gone, NEVER updated by `switchToTab`) instead of `primaryTab` — so even if `_ensureInitialPagePooled` runs after a switch has moved `primaryTab`, it pools the auto-attached Page under its own (now-closed) tabId rather than mis-pooling under the destination.
