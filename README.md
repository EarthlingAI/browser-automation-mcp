...
**Sticky keeps the debugger attached.** A `lifetime:"sticky"` arming keeps the debugger pinned to the tab for the duration (so the "Started debugging this browser" infobar stays visible, like `browser_activate_tab(level:"render")` does) — this is what lets it answer page-driven dialogs from timers, network responses, or `beforeunload`. Without sticky-stay-attached, the debugger's idle backstop could drop those dialogs onto the safe-default DISMISS, which is the wrong answer for an explicit sticky-accept. A `lifetime:"one_shot"` arming relies on the normal lease-scoped hold: the debugger is held for the lease (the infobar appears once and stays steady, not flickering per action) and self-detaches only on lease release, tab close, or the ~180 s idle backstop — on the assumption you armed right before the click that triggers the dialog.
