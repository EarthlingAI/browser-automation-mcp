 ...
 *
 * Driven by:
 *   - globalThis.__earthlingIndicator.set({state, agentLabel})  ← extension push on lease edges
 *   - globalThis.__earthlingIndicator.onAction({phase, kind, ref})  ← helpers.js per-action edges
 */

(() => {
  if (globalThis.__earthlingIndicator?.teardown) {
    try {
      globalThis.__earthlingIndicator.teardown();
    } catch {}
  }
  ...
  const DEFAULT_AGENT_NAME = "Earthling";
...
`;
  ...
  let log = [];
  const tabKey = `earthling-log-${location.hostname}-${location.pathname}`;

  buildShadow();
  void hydrateLog();
  watchStorage();

  globalThis.__earthlingIndicator = {
    set(next) {
      const prev = state;
  ...
  function buildShadow() {
    ...
    host.id = "__earthling_indicator_host__";
    host.style.cssText =
  ...
  function buildConstellationSvg(size) {
    ...
    edgeGrad.setAttribute("id", "earthling-edge");
    ...
    nodeGrad.setAttribute("id", "earthling-node");
    ...
    edges.forEach(([x1, y1, x2, y2], i) => {
      ...
      line.setAttribute("stroke", "url(#earthling-edge)");
    ...
    nodes.forEach(([cx, cy], i) => {
      ...
      c.setAttribute("fill", "url(#earthling-node)");
  ...
  function resolveTarget(ref) {
    ...
    try {
      return globalThis.__earthlingResolveRef?.(ref) || null;
    } catch {
      return null;
    }
  }
  ...
  function teardown() {
    ...
    restoreFavicon();
    host?.remove();
    globalThis.__earthlingIndicatorLoaded = false;
  }
