...
18. **Per-client tool dispatch is serialized.** `BrowserBackend.callTool` holds a per-instance mutex (`_dispatchMutex`) across the whole body — pre-dispatch `isConnected` check, `tool.handle`, and `response.serialize`. This prevents parallel `tool_use` blocks from racing over the shared `Browser`/CDP-session state: the root failure was `earthlingTabs.relaySend` opening a fresh `newBrowserCDPSession` per call, with N concurrent callers all crashing on the same mid-flight WS close with `-32000`. `_reconnectInFlight` still coalesces concurrent disconnect observations but is redundant under the mutex. When the mutex actually queues a call, the MCP process bumps the `concurrent_dispatch_serialized` counter via `POST /telemetry/bump` so `/health` surfaces non-zero values whenever parallel dispatch is happening in production. A separate defensive counter `switch_tab_target_mismatch` is bumped by `_handleSetAutoAttach` exit whenever the declared switch target (tracked in `CdpRelayServer._declaredSwitchTarget`, cleared only on tab-gone/release events) disagrees with the tab the client actually landed on — regression signal for the Scenario-1 hint-chain bypass class.

19. **Daemon-owned auto-blanks auto-close.** The Priority-3 fallback in `_handleSetAutoAttach` opens an `about:blank` when no hint is available. That tab is tracked in `CdpRelayServer._autoOpenedBlanks` keyed by clientId and closed automatically on three paths: (a) `switchToTab` when the client switches off it, (b) `releaseTab` when the client releases it, (c) client-WS close (disconnect). Without this tracking, every `connectOverCDP` handshake leaks one blank into the user's browser for the lifetime of the browser session. `_lastSwitchedTab` and `_declaredSwitchTarget` are explicitly NOT cleared on client-disconnect — they are designed to survive the WS-close → reconnect cycle and are cleared only on actual tab-gone events (`_forgetTab`, `releaseTab`, `revokeTab`, `onExtensionLost`).

20. **`listBrowserTabs` uses `tab.url || tab.pendingUrl`.** A tab that was just `chrome.tabs.create({url:X})`'d has `tab.url === ''` for ~50ms until navigation commits. Before this fix, `browser_list_all_tabs` (and every spec calling `Earthling.listTabsAnnotated`) could race with `Earthling.openTab` + `Earthling.switchToTab` and transiently not see the just-opened tab, causing spurious test failures. `Earthling.queryOrphanBlanks` must query `chrome.tabs.query({})` and filter in JS — the `{url}` option uses match-pattern syntax which does NOT match the `about:` scheme, so `chrome.tabs.query({url:"about:blank"})` returns 0 matches.
...
- **Chrome-only unit + integration suite:** `npm run ctest` (in `packages/playwright-mcp/`). Phase 2/3 additions under `packages/playwright-mcp/tests/`:
  ...
  - `switch-tab-target-mismatch-counter.spec.ts` — `/health.telemetry.switch_tab_target_mismatch` field is wired and monotonic across a client connect cycle.
  - `orphan-sweep.spec.ts` — `Earthling.queryOrphanBlanks` extension command returns the documented shape (queries `chrome.tabs.query({})` + filters in JS); dev-only sweep helper is reachable through the protocol seam.

**Test tab cleanup:** Daemon auto-closes Priority-3 auto-opened blanks on client disconnect / switch-off / release (invariant #19). Test specs only need to close their explicitly-opened tabs via `Earthling.closeTab`; they do NOT need to track the auto-attach blank the daemon spawned during `connectOverCDP` — that cleanup is free.
- **Dev-only env flags:**
  - `EARTHLING_MCP_SWEEP_ORPHANS=1` — on daemon startup (after extension pair), close accumulated `about:blank` tabs older than 5 min with no history and no lease. Default off. Sweep is fire-and-forget; failure never blocks startup.
